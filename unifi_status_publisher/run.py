...
class UnifiStatusPublisher(object):
    stainfo_re = re.compile(
        "^(?P<timestamp>.{2,3})\[(?P<mac>.*)\]\s*(?P<flags>.+)\s+idle=(?P<idletime>[0-9 ]+)rssi=(?P<rssi>[0-9 ]+)\s+(?P<bandwidth_in>[0-9 ]+)/(?P<bandwidth_out>[0-9 ]+)ccq=.*queued=(?P<tx>[0-9 ]+)/(?P<tx_ret>[0-9 ]+)/(?P<tx_queued>[0-9 ]+)rx/ret/mc=(?P<rx>[0-9 ]+)/(?P<rx_ret>[0-9 ]+)/(?P<rx_queued>[0-9 ]+)err=(?P<err>[0-9/ ]+)$")

    def __init__(self):
        self.redis_instance = redis.StrictRedis()
        self.client = paramiko.client.SSHClient()
        self.client.load_system_host_keys()
        self.client.connect(local_settings.UNIFI_IP, username=local_settings.UNIFI_USERNAME,
                            password=local_settings.UNIFI_PASSWORD)
        ...
        for line in stdout:
            ...
            if data.startswith("- showing"):
                ...
                self.redis_instance.publish("home:broadcast:generic", json.dumps(
                    {"key": "unifi-status", "content": publish_data}))
                publish_data = {"devices": [], "stats": {}}
            ...
            item_data["bandwidth_in"] = int(
                matched.group("bandwidth_in").strip())
            item_data["bandwidth_out"] = int(
                matched.group("bandwidth_out").strip())
