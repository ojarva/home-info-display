...
class UnifiStatusPublisher(object):
    stainfo_re = re.compile("(?P<timestamp>.{2,3})\[(?P<mac>.*)\] (?P<flags>.{3}) idle=(?P<idletime>[0-9 ]+)rssi=(?P<rssi>[0-9 ]+) (?P<current_bandwidth>[0-9 ]+)/(?P<maximum_bandwidth>[0-9 ]+)ccq=.*queued=(?P<tx>[0-9 ]+)/(?P<ret>[0-9 ]+)/(?P<queued>[0-9 ]+)rx=(?P<rx>[0-9 ]+)err=(?P<err>[0-9/ ]+)")
    ...
    def poll_unifi_status(self):
        ...
        for line in stdout:
            ...
            item_data["current_bandwidth"] = int(matched.group("current_bandwidth").strip())
            item_data["maximum_bandwidth"] = int(matched.group("maximum_bandwidth").strip())
