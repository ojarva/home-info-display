...
class NotificationsInfo(View):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        obj.delete()
        redis_instance.publish("%s-pubsub" % item_type,
...
class NotificationUpdate(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        item.can_dismiss = request.POST.get("can_dismiss") in [
            "1", "True", "true"]
        item.level = request.POST.get("level", "normal")
        if "from_now_timestamp" in request.POST:
            item.from_now_timestamp = dateutil.parser.parse(
                request.POST.get("from_now_timestamp"))
        else:
            item.elapsed_since = None
        if "elapsed_since" in request.POST:
            item.elapsed_since = dateutil.parser.parse(
                request.POST.get("elapsed_since"))
        else:
            item.elapsed_since = None
