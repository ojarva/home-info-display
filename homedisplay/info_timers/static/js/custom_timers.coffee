CustomTimer = (options) ->
  ...
  nullTimeField = (content, field) ->
    replace_string = ""
    #TODO: validate this
    for i in [0..time_spec["#{field}_l"]]
      replace_string += "0"
  ...
  processButton = (content) ->
    ...
    if content.substring(0, 6) == "clear-"
      # backspace
      clear_field = content.substring(6)
  ...
  modal_elem.find(".add-timer-button").on "click", ->
    content_switch.userActivity()
    content = jq(this).data("content").trim()
    processButton content
...
jq =>
  this.custom_timer = new CustomTimer
