    ...
    jq.each data.next, ->
      ...
      if first_date == false
        ...
        if first_date != @item.date
          next_weather_elem.addClass "new-day"
          new_day = true

      next_weather_elem.find(".timestamp").html @name
      next_weather_elem.find(".temperature").html @item.feels_like
      next_weather_elem.find(".symbol").html "<img src='/homecontroller/static/images/#{@item.icon}.png'>"
      next_weather_elem.find(".temperature-unit").html "&deg;C"
      current_index += 1
